...
let phrase = "ALL THAT GLITTERS ISNT GOLD"
...
letterLs.forEach(L => {
    L.addEventListener('click', function addL (){
        ...
        if(hasText){
            ...
            document.querySelector('.spaceTwentySix').style.color = 'rgba(0, 0, 0, 100)'
        }
    })
})
letterTs.forEach(T => {
...
letterDs.forEach(D => {
    D.addEventListener('click', function addD (){
        ...
        if(hasText){
            document.querySelector('.spaceTwentySeven').style.color = 'rgba(0, 0, 0, 100)'
        }
    })
})
letterNs.forEach(N => {
